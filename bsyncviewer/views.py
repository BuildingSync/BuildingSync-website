...
GITHUB_VERSION_MAP = {"1.0.0": "v1.0", "2.0.0": "v2.0"}
GITHUB_RELEASE_TAG_URL = "https://github.com/BuildingSync/schema/releases/tag/{version}"
GITHUB_RELEASE_DOWNLOAD_URL = (
    "https://github.com/BuildingSync/schema/releases/download/{version}/{resource}"
)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {
                    "success": False,
                    "error": "No schema matching schema_version {} in database".format(
                        version
                    ),
                }
            )
...
def index(request):
    ...
    try:
        version = (
            subprocess.check_output(["git", "describe", "--tags"], cwd=FILE_DIR)
            .decode("utf-8")
            .strip()
        )
    except Exception:
        ...
        releases.append(
            {
                ...
                "xsd_url": GITHUB_RELEASE_DOWNLOAD_URL.format(
                    version=github_version, resource="BuildingSync.xsd"
                ),
                "data_dict_xlsx_url": GITHUB_RELEASE_DOWNLOAD_URL.format(
                    version=github_version, resource="DataDictionary.xlsx"
                ),
            }
        )
...
def use_cases(request):
    ...
    sorted_versions = sorted(
        schemas, key=lambda x: semantic_version.Version(x), reverse=True
    )

    for s in sorted_versions:
        schema = Schema.objects.filter(version=s).first()
        ucs = list(
            UseCase.objects.filter(make_public=True, schema=schema)
            .order_by("name")
            .values()
        )
        cases[s] = ucs

    context = {"user_usecases": user_usecases, "sorted_public_cases": cases}
...
def retrieve_additional_dictionary_data(request):
    ...
    if enum_ids.count() > 0:
        ...
        has_enum = True
        enum_results = (
            Enumeration.objects.filter(
                enumeration_class_id=enum_ids[0].enumeration_class_id
            )
            .only("name")
            .order_by("pk")
        )
        ...
        for item in enum_results:
            ...
            bedes_mappings = BedesEnumerationMapping.objects.filter(
                enumeration_id=item.id
            )
            if bedes_mappings.count() > 0:
                # take first
                bedes_enum = model_to_dict(
                    BedesEnumeration.objects.get(
                        pk=bedes_mappings[0].bedesEnumeration_id
                    )
                )
    ...
    data = {"bedes_terms": bedes_terms, "has_enum": has_enum, "enums": enums}
...
def enumerations(request, version):
    ...
    if not schema.enumerations_file:
        return HttpResponseRedirect(
            reverse_lazy("enumerations", args=[DEFAULT_SCHEMA_VERSION])
        )

    enumerations_data = json.load(schema.enumerations_file)
    # remove measures
    enumerations_data = [
        term for term in enumerations_data if term["name"] != "MeasureName"
    ]
...
def measures(request, version):
    ...
    if not schema.enumerations_file:
        return HttpResponseRedirect(
            reverse_lazy("measures", args=[DEFAULT_SCHEMA_VERSION])
        )
...
def get_schema_to_examples() -> Dict[str, List[pathlib.Path]]:
    ...
    for schema_dir in validator_dir.glob("*"):
        schema_to_examples[schema_dir.name.replace("schema", "", 1)] = [
            (p.name, str(p)) for p in schema_dir.glob("*.xml")
        ]
        ...
        if form_type == "file":
            ...
            if "download" not in request.POST:
                ...
                filepath = os.path.join(
                    ...
                    "validator",
                    "examples",
                    "schema" + request.POST["schema_version"],
                    filename,
                )
...
def download_usecase_example(request):
    file_path = os.path.join(
        ...
        "lib",
        "use_cases",
        "example_usecase_definitions.sch",
    )
...
def download_usecase_file(request, pk):
    ...
    if os.path.exists(the_file.path):
        with open(the_file.path, "rb") as fh:
            ...
            response["Content-Disposition"] = (
                'attachment; filename="%s"' % os.path.basename(the_file.path)
            )
            return response
    raise Http404
...
def update_user(request):
    if request.POST:
        ...
        if form.is_valid():
            ...
            return render(
                request, "registration/updateuser.html", {"update_user_form": form}
            )
    else:
...
def successView(request):
    ...
    messages.add_message(
        request, messages.SUCCESS, "Email Sent! Thank you for your message."
    )
