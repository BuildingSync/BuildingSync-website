...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--schema_version", type=str, default=settings.DEFAULT_SCHEMA_VERSION
        )
    ...
    def handle(self, *args, **options):
        ...
        if save_to_db:
            self.stdout.write(
                "Saving BEDES mappings to database from already generated mapping files."
            )
    ...
    def parse(self, bedes_version, schema_version):
        ...
        the_path = os.path.join(
            os.path.dirname(__file__), "../../lib/bedes", bedes_version
        )
        ...
        for attribute in schema.attributes.all().order_by("id"):
            ...
            for bt in bedes.terms:
                ...
                distance = jellyfish.jaro_winkler_similarity(
                    bsync_term.lower(), bt["Term"].lower()
                )
            ...
            results[attribute.id] = sorted(
                results[attribute.id], key=lambda k: -k["distance"]
            )
                    ...
                    while restart_process:
                        ...
                        for number_of_words in range(len(bsync_words) - 1, 1, -1):
                            ...
                            for starting_index in range(
                                ...
                                for i in range(
                                    starting_index, starting_index + number_of_words
                                ):
                                    if bsync_words[i][1]:
                                ...
                                for bt in bedes.terms:
                                    ...
                                    if distance >= 0.98:
                                        for i in range(
                                            ...
                                            word_groups[word_group] + number_of_words,
                                        ):
                        ...
                        if term_match_status == "Matched-Term":
                            ...
                            if lo_match_status == "Matched-Term":
                                ...
                                if term_match_status == "Matched-Word":
                                    ...
                                    words_data["matched_word_example_URL"].append(
                                        term_match_URL
                                    )
                ...
                results[attribute.id].append(
                    {
                        ...
                        "matched_word_example_URL": words_data[
        ...
        self.stdout.write(
            "*******There are {} unique BEDES terms to add*******".format(unique_cnt)
        ...
        with open("%s/bsync_unique_words.csv" % (the_path), "w", newline="") as file:
            ...
            writer.writerow(
                [
                    ...
                    "Unique Words Matched to Words",
                    "Unique Unmatched Words",
                ]
            )
        ...
        for enumeration in Enumeration.objects.filter(schema=schema):
            ...
            for be in bedes.enumerations:
                ...
                if not be.get("List-Option") or not isinstance(be["List-Option"], str):
                    continue
                distance = jellyfish.jaro_winkler_similarity(
                    enumeration.name, be["List-Option"]
                )
                    ...
                    results[enumeration.id].append(
                        {
                            ...
                            "bedes_object": be,
                            "distance": distance,
                            "associated_attribute_ids": " ".join(
                                [str(item) for item in associated_attrs]
                            ),
                        }
                    )
            results[enumeration.id] = sorted(
                results[enumeration.id], key=lambda k: -k["distance"]
            )
        ...
        content_uuids = []
        with open(
            "%s/bedes-mappings-enumerations.csv" % (the_path),
            "w",
            encoding="utf-8",
            newline="",
        ) as file:
        ...
        unique_cnt = len(list(list_set))
        self.stdout.write(
            "*******There are {} unique BEDES enum values to add*******".format(
                unique_cnt
            )
        )
    ...
    def word_matching(self, bsync_word, term_dict):
        bsync_word = (
            bsync_word.lower()
        )  # converting to all same case to neutralize case differences in matching
        ...
        for term in term_dict:
            ...
            if (
                bsync_word == term[term_key].lower()
                or bsync_word == term[term_key].lower() + "s"
            ):
                match_URL = term["URL"]
        ...
        if match_status == "Unmatched":
            ...
            for term in term_dict:
                ...
                if term_key == "Term":
                    bedes_words = self.acronym_check(
                        re.findall("[A-Z][^A-Z ]*", term[term_key])
                    )
                else:
                    bedes_words = re.findall(
                        "[^:^(^)^,^ ][^,^ ^:^(^)]*", term[term_key]
                    )
        ...
        while restart_process:
            ...
            for number_of_words in range(len(bsync_words), 0, -1):
                ...
                for word_group in word_groups:
                    if word_group in manual_mappings.keys() or (
                        word_group[-1] == "s"
                        and (word_group[:-1] in manual_mappings.keys())
                    ):
    ...
    def acronym_check(self, word_list):
        ...
        word_list.append(
            "buffer"
        )  # arbitrary string added to end of list to help check run properly.  Deleted after check has run
    ...
    def save_mappings_to_database(self, bedes_version, schema_version):
        ...
        with open(
            os.path.join(
                ...
                "../../lib/bedes",
                bedes_version,
                "bedes_online_dictionary_uuid-terms.xml",
            )
        ) as file:
        ...
        for term in bedes_mappings:
            ...
            if term["matched_term_URL"] != "":
                ...
                for item in items:
                    ...
                    match = next(
                        (i for i in xml_dict["nodes"]["node"] if i["URL"] == item),
                        False,
                    )
                    ...
                    if match:
                        ...
                        the_term, _ = BedesTerm.objects.get_or_create(
                            content_uuid=match["Content-UUID"], term=match["Term"]
                        )
        ...
        for term in bedes_mappings:
            ...
            if term["match_type"] != "all Unmatched":
                ...
                attributes = Attribute.objects.filter(
                    id=term["attribute_id"], schema=schema
                )
                terms = []
                if term["bedes_content_uuid"] != "":
                    terms = BedesTerm.objects.filter(
                        content_uuid=term["bedes_content_uuid"]
                    )
                    ...
                    tlist = [
                        i.strip(" ").replace("https://bedes.lbl.gov/node/", "")
                        for i in tlist
                    ]
        ...
        for term in bedes_mappings:
            ...
            if term["distance"] != "":
                ...
                terms = BedesEnumeration.objects.filter(
                    content_uuid=term["bedes_content_uuid"]
                )
                ...
                if terms.count() > 0 and enums.count() > 0:
                    ...
                    BedesEnumerationMapping.objects.get_or_create(
                        bedesEnumeration=bedes_term, enumeration=enum
                    )
