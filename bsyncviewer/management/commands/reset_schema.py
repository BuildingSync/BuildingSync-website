    ...
    def handle(self, *args, **options):
        ...
        if not options["overwrite"]:
            if Schema.objects.filter(version=options["schema_version"]).exists():
                self.stdout.write(
                    "Schema already exists in the database: %s"
        ...
        sf = (
            "bsyncviewer/lib/buildingsync_schemas/BuildingSync_v%s.xsd"
            % options["schema_version"]
        )
        schema_file = (
            "bsyncviewer/media/uploaded_schemas/BuildingSync_v%s.xsd"
            % options["schema_version"]
        )
