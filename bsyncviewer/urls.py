...
urlpatterns = [
    ...
    path(
        "user/logout/", auth_views.LogoutView.as_view(next_page="login"), name="logout"
    ),
    ...
    path(
        "use_case/<int:pk>/update/",
        views.UseCaseUpdate.as_view(),
        name="use_case_update",
    ),
    path(
        "use_case/<int:pk>/delete/",
        views.UseCaseDelete.as_view(),
        name="use_case_delete",
    ),
    re_path("download_examples", views.download_examples, name="download_examples"),
    re_path(
        "download_usecase_example",
        views.download_usecase_example,
        name="download_usecase_example",
    ),
    re_path(
        "download_usecase_file/(?P<pk>[0-9]+)/$",
        views.download_usecase_file,
        name="download_usecase_file",
    ),
    ...
    re_path(
        r"^dictionary/(?P<version>[0-9a-zA-Z_\.-]+)/$",
        views.dictionary,
        name="dictionaryversion",
    ),
    re_path(
        r"^ajax/enum/$",
        views.retrieve_additional_dictionary_data,
        name="get_additional_data",
    ),
    re_path(
        r"^enumerations/(?P<version>[0-9a-zA-Z_\.-]+)/$",
        views.enumerations,
        name="enumerations",
    ),
    re_path(
        r"^measures/(?P<version>[0-9a-zA-Z_\.-]+)/$", views.measures, name="measures"
    ),
...
if settings.DEBUG:
    ...
    urlpatterns += [
        re_path(r"^documentation/(?P<version>[0-9a-zA-Z_\.-]+)/$", views.documentation)
    ]
