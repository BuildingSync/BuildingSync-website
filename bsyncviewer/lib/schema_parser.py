...
class BuildingSyncSchemaProcessor(object):
    ...
    def _read_complex_type(self, parent_object):
        ...
        for child in parent_object:
            if child.tag.endswith("sequence"):
                ...
                this_complex_type.simple_contents.append(
                    self._read_simple_content(child)
                )
            elif child.tag.endswith("attribute"):
    ...
    def _read_simple_content(self, parent_object):
        ...
        for child in parent_object:
            if child.tag.endswith("extension"):
                ...
                raise Exception(
                    "Invalid tag type in _read_simple_content: " + child.tag
                )
    ...
    def _walk_named_element(
        self, parent_element, root_path, current_tree_level, current_index
    ):
        return_rows = []
        num_added = 0
    ...
    def _walk_sequence_or_choice_element(
        self, parent_element, root_path, current_tree_level, current_index
    ):
        return_rows = []
        num_added = 0
            ...
            if elem.type:
                ...
                return_rows.append(
                    {
                        "name": elem.name,
                        "description": "; ".join(
                            [anno.documentation for anno in elem.annotations]
                        ),
                        "type": elem.name,
                ...
                if instance:
                    if isinstance(instance, SimpleTypeElement):
                        ...
                        this_num_added, new_rows, potential_doc_string = (
                            self._walk_named_element(
                                instance,
                                root_path + "." + elem.name,
                                current_tree_level + 1,
                                current_index,
                        ...
                        raise Exception(
                            "Couldn't grok the type of the searched reference element; type: %s"
                            % elem.type
                        )
                    current_index += this_num_added
                    num_added += this_num_added
        ...
        for elem in parent_element.ref_elements:
            ...
            this_num_added, new_rows, potential_doc_string = (
                self._walk_reference_element(elem)
            )
            current_index += this_num_added
            num_added += this_num_added
            return_rows.extend(new_rows)
    ...
    @staticmethod
    def _walk_restriction_element(
        parent_element, root_path, current_tree_level, current_index
    ):
        return_rows = []
        num_added = 0
