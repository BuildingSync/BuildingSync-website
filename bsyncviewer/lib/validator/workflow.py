...
class ValidationWorkflow(object):
    ...
    def validate_schema(self):
        ...
        if self.schema.schema_file:
            my_schema = xmlschema.XMLSchema(
                self.schema.schema_file.path, validation="lax"
            )
    ...
    def _format_error(self, errors):
        ...
        for err in errors:
            ...
            if path.split("/")[-1] == "UsefulLife" and reason.startswith(
    ...
    def validate_use_cases(self):
        ...
        resp["use_cases"] = OrderedDict(
            sorted(
                processed_use_cases.items(), key=lambda t: f"{not t[1]['valid']} {t[0]}"
            )
        )
        ...
        try:
            for path in paths:
                ...
                if loopingVar.__class__.__name__ == "OrderedDict":
                    ...
                    for index, loop_var in enumerate(loopingVar):
                        if fpath not in loop_var and isinstance(loop_var, list):
                            # add another loop
                            # print("L is a list!")
                            loopingVar = []
                            for x in loop_var:
                            ...
                            loopingVar[index] = loop_var[fpath]
